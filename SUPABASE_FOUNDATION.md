...
All ecosystem services MUST use Supabase as their source of truth:

| Service | Auth | Database | Storage | Realtime | Status |
|---------|------|----------|---------|----------|--------|
| `logic.logline.world` (CLI) | ✅ JWT | ✅ PostgREST | ✅ Buckets | — | Integrated |
| `obs-api.logline.world` | ✅ JWT | ✅ Drizzle | ✅ Buckets | ✅ Channels | Integrated |
| `llm-gateway.logline.world` | ⏳ JWT | ⏳ fuel_events | — | — | Pending |
| `code247.logline.world` | ⏳ JWT | ⏳ jobs/checkpoints | — | ⏳ Broadcast | Pending |

See `INTEGRATION_PHASES.md` for migration plan.
...
```bash
# Supabase connection (required for all)
SUPABASE_URL="https://aypxnwofjtdnmtxastti.supabase.co"
SUPABASE_ANON_KEY="<anon-key>"
SUPABASE_SERVICE_KEY="<service-role-key>"  # Server-only, NEVER in client/browser
```

### Next.js Services (obs-api)
...
### Rust Services (llm-gateway, code247)

Rust services use `SUPABASE_URL` + `SUPABASE_ANON_KEY` via the `logline-supabase` crate.
Service-to-service calls use `SUPABASE_SERVICE_KEY` for elevated privileges.

Notes:

- Use pooled URL (port 6543) for runtime and unpooled (port 5432) for migrations.
- Do not commit real secrets — use `.env` files excluded in `.gitignore`.
...
- `logic.logline.world/supabase/migrations/20260227000002_rbac_rls.sql`

How:

1. Run via Supabase CLI: `supabase db push`
2. Or paste in SQL editor for project `aypxnwofjtdnmtxastti`
