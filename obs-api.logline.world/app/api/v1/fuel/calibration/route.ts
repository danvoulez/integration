...
import { fuelCalibrationQuerySchema, getFuelCalibration } from '@/lib/obs/fuel';
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const parsedQuery = fuelCalibrationQuerySchema.safeParse({
  ...
  try {
    const payload = await getFuelCalibration(parsedQuery.data);
    return successEnvelope(requestId, payload);
  } catch (error) {
    return errorEnvelope(requestId, 500, 'FUEL_CALIBRATION_FAILED', 'failed to compute fuel calibration', {
