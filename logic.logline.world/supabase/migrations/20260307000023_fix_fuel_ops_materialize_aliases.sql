-- Migration 023: qualify materialize_fuel_ops_jobs source columns
-- Depends on: 022_fix_fuel_ops_materialize
...
begin
  if p_job_name in ('baseline_snapshot', 'baseline_and_alerts') then
    ...
    select
      ...
      fw.policy_version,
      case
        when sum(case when fw.envelope_status = 'above_p95' then fw.event_count else 0 end) > 0 then 'warn'
        else 'ok'
      end,
      p_reference_time,
      now(),
      jsonb_build_object(
        'window_from', min(fw.window_start),
        'window_to', max(fw.window_start),
        'event_count', sum(fw.event_count),
        'above_p95_count', sum(case when fw.envelope_status = 'above_p95' then fw.event_count else 0 end),
        'above_p75_count', sum(case when fw.envelope_status = 'above_p75' then fw.event_count else 0 end),
        'cold_start_count', sum(case when fw.envelope_status = 'cold_start' then fw.event_count else 0 end),
        'fallback_count', sum(fw.fallback_count),
        'timeout_count', sum(fw.timeout_count)
      ),
      jsonb_build_object(
        'baseline_ready', bool_or(fw.envelope_status <> 'cold_start'),
        'latest_window_start', max(fw.window_start)
      )
  ...
  if p_job_name in ('alerts_snapshot', 'baseline_and_alerts') then
    ...
    select
      ...
      fac.app_id,
      fac.policy_version,
      case when count(*) filter (where fac.severity = 'critical') > 0 then 'failed'
      ...
      jsonb_build_object(
        'open_alerts', count(*),
        'critical_alerts', count(*) filter (where fac.severity = 'critical'),
        'warn_alerts', count(*) filter (where fac.severity = 'warn')
      ),
      jsonb_build_object(
        'codes', coalesce(jsonb_agg(fac.alert_code order by fac.alert_code), '[]'::jsonb),
        'latest_detected_at', max(fac.detected_at)
      )
