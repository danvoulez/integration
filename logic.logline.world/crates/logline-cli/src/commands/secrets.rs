...
pub fn store_credential(key: &str, value: &str) -> anyhow::Result<()> {
    let _keychain_ok = keyring::Entry::new(KEYRING_SERVICE, key)
        .ok()
        .and_then(|e| e.set_password(value).ok())
        .is_some();

    let dir = crate::supabase::config_dir().join("secrets");
...
pub fn load_credential(key: &str) -> Option<String> {
    if let Some(val) = keyring::Entry::new(KEYRING_SERVICE, key)
        .ok()
        .and_then(|e| e.get_password().ok())
    {
        return Some(val);
    }
    let path = crate::supabase::config_dir().join("secrets").join(key);
...
pub fn cmd_secrets(command: SecretsCommands, json: bool) -> anyhow::Result<()> {
    match command {
        ...
        SecretsCommands::Doctor => cmd_secrets_doctor(json),
    }
}
...
fn cmd_secrets_doctor(json: bool) -> anyhow::Result<()> {
    ...
    let profile = identity
        .as_ref()
        .map(|i| i.profile.as_str())
        .unwrap_or("none");
    let subject_email = identity
        .as_ref()
    ...
    let ready_for_infra =
        vault_ok && session_ok && logged_in && passkey_ok && !is_founder && no_leaks;
    ...
    if !logged_in {
        ...
        let auth_mark = if passkey_ok && !is_founder {
            "✓"
        } else {
            "✗"
        };
        ...
        println!(
            "    {method_mark} method: {auth_method}{}",
            if !passkey_ok {
                "  <-- FAIL: must be passkey. Run: logline auth login --passkey"
            } else {
                ""
            }
        );
        ...
        println!(
            "    {profile_mark} profile: {profile}{}",
            if is_founder {
                "  <-- FAIL: founder cannot run infra. Use operator/service account."
            } else {
                ""
            }
        );
    ...
    if ready_for_infra {
        ...
        println!();
        if !vault_ok {
            println!("  Fix: logline secrets set <key> (see vault above)");
        }
        if !session_ok {
            println!("  Fix: logline auth unlock");
        }
        if !logged_in {
            println!("  Fix: logline auth login --passkey");
        } else if !passkey_ok {
        ...
        if is_founder {
            println!("  Fix: log in as operator/service user, not founder");
        }
        if !no_leaks {
            println!("  Fix: remove secrets from environment variables (see env section above)");
        }
    }
