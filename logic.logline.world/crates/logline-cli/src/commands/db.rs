...
fn cmd_migrate_up(env: &str, json: bool) -> anyhow::Result<()> {
    ...
    eprintln!(
        "Applying {} pending migration(s) to {env}...",
        pending.len()
    );
...
fn cmd_migrate_review(json: bool) -> anyhow::Result<()> {
    ...
    crate::pout(
        ...
        &format!(
            "{} migration(s) reviewed. Receipt valid until {}.",
            pending_names.len(),
...
fn cmd_migrate_apply(env: &str, json: bool) -> anyhow::Result<()> {
    ...
    let receipt_json = secrets::load_credential(REVIEW_RECEIPT_KEY).ok_or_else(|| {
        anyhow::anyhow!(
    ...
    eprintln!(
        "Review receipt valid. Reviewed: {}",
        reviewed_migrations.join(", ")
    );
    ...
    let _ = secrets::store_credential(
        REVIEW_RECEIPT_KEY,
...
fn cmd_verify_rls(env: &str, json: bool) -> anyhow::Result<()> {
    ...
    if issues.is_empty() {
        ...
        bail!(
            "RLS verification failed: {} critical issue(s), {} warning(s). Deploy blocked.",
            issues.len(),
            warnings.len()
        )
    }
