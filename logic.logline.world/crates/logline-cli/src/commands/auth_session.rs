...
pub fn require_unlocked() -> anyhow::Result<SessionToken> {
    let session = load_session()
        .ok_or_else(|| anyhow::anyhow!("Session locked. Run `logline auth unlock` first."))?;
    ensure!(
...
pub fn cmd_auth_session(command: SessionCommands, json: bool) -> anyhow::Result<()> {
    match command {
        ...
        SessionCommands::Status => match load_session() {
            Some(s) if s.expires_at > now_secs() => {
                ...
                crate::pout(
                    ...
                    &format!("Unlocked — {mins}m {secs}s remaining. ID: {}", s.session_id),
                )
            }
            Some(_) => {
                delete_session()?;
...
fn check_founder_remote(user_id: &str) -> anyhow::Result<bool> {
    ...
    let anon_key =
        secrets::load_credential_or_env("supabase_anon_key", "NEXT_PUBLIC_SUPABASE_ANON_KEY")
            .ok_or_else(|| anyhow::anyhow!("supabase_anon_key not configured"))?;

    let auth = crate::supabase::load_auth().ok_or_else(|| anyhow::anyhow!("Not logged in"))?;
    ...
    let is_founder = body.as_array().is_some_and(|arr| !arr.is_empty());
