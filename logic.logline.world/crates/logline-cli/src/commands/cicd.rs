...
fn cmd_cicd_run(
    ...
    let steps = file
        ...
        .ok_or_else(|| {
            ...
            anyhow::anyhow!("Pipeline '{name}' not found. Available: {available:?}")
        })?
        .clone();
        ...
        match outcome {
            ...
            Err(e) => {
                ...
                if file.on_failure == "abort" {
                    ...
                    let principal = identity.as_ref().map(|id| {
                        serde_json::json!({
                            "user_id": id.user_id,
                            "email": id.email,
...
fn cmd_cicd_status(json: bool) -> anyhow::Result<()> {
    ...
    if !missing_secrets.is_empty() {
        println!(
            "\n⚠ {} secret(s) missing — pipeline will fail.",
            missing_secrets.len()
        );
    }
