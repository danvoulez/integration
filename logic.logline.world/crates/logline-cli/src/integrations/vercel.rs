...
fn vercel_client() -> anyhow::Result<(reqwest::blocking::Client, String, String, String)> {
    ...
    let project_id = secrets::require_credential_or_env("vercel_project_id", "VERCEL_PROJECT_ID")?;
...
pub fn deployment_status() -> anyhow::Result<serde_json::Value> {
    ...
    let url = format!("https://api.vercel.com/v6/deployments?projectId={project_id}&limit=1");
